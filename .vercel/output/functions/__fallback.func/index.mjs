globalThis.__nitro_main__ = import.meta.url; globalThis.__nitro_main__ = import.meta.url; function setupVite({ manifest, services }) {
...
const findRouteRules = (m,p)=>{let r=[];if(p[p.length-1]==='/')p=p.slice(0,-1)||'/';let s=p.split('/');s.length-1;if(s[1]==="assets"){r.unshift({data:[{name:"headers",route:"/assets/**",handler:headers,options:{"cache-control":"public, max-age=31536000, immutable"}}],params:{"_":s.slice(2).join('/'),}});}return r;};



const _lazy_mAaMHo = defineLazyEventHandler(() => import('./chunks/_/ssr-renderer.mjs'));

const findRoute = (m,p)=>{if(p[p.length-1]==='/')p=p.slice(0,-1)||'/';let s=p.split('/');s.length-1;return {data:{route:"/**",handler:_lazy_mAaMHo},params:{"_":s.slice(1).join('/'),}};};
